...
export const ClientLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row text-slate-900 font-sans">
      ...
      {isImpersonating && (
        <div className="fixed top-0 inset-x-0 z-50 bg-amber-500 text-slate-950 px-4 py-2 text-xs font-semibold flex items-center justify-between shadow-md">
          <div className="flex items-center gap-2">
            ...
            <span>You are viewing as client: <strong>{user?.full_name}</strong></span>
          </div>
          <button
            onClick={() => {
              stopImpersonation();
            ...
            className="inline-flex items-center gap-1.5 bg-slate-950 text-white px-2.5 py-1 rounded-md text-xs hover:bg-slate-800 transition-colors"
          >
      ...
      {/* Mobile Topbar */}
      <div
        className={`md:hidden flex items-center justify-between p-4 bg-white border-b border-slate-200 sticky top-0 z-40 ${
          ...
          <div className="w-64 max-w-full bg-white h-full p-4 flex flex-col justify-between shadow-2xl">
            <div>
              <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-3">
                <span className="font-bold text-sm text-slate-900">Client Portal</span>
                <button
